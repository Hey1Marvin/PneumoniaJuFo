...
#model_zustand  = torch.load("resnet_model34NIH.pth")
...
class PneumoniaDataset(Dataset):
    ...
    def __getitem__(self, index):
        img_path = os.path.join(self.image_dir, self.annotations.iloc[index, 0] + '.dcm')
...
# Datensatz erstellen
dataset = PneumoniaDataset(image_dir, csv_file, transform=transform)

# Datensatz in Trainings- und Validierungsdatensatz aufteilen
...
prevacc = False
# Trainieren und Evaluieren des Netzwerks
for epoch in range(2,50):  # Anzahl der Epochen anpassen
    resnet.train()  # Trainingsmodus
    print("Epoche: ", epoch)
    progBar = progress(len(train_loader))
    history = []
    ...
    with open("historyResNet34NIH.txt", "a") as file:
    ...
    if prevacc is False or prevacc < accuracy:
        torch.save(resnet.state_dict(), 'resnet_model34NIH.pth')
